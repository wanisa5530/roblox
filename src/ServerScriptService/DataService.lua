...
local D = { cache = {} }
local function default() return { cash = Config.StartingCash, foods = { MooPing = true }, total = 0, rep = 0, served = 0, lastClaim = 0, streak = 0, staff = {}, upg = {}, quests = nil, level = 1, decor = { owned = {}, tent = "TentRed", chairs = "ChairPlastic", sign = "SignClassic" } } end
function D.load(p)
	local ok, data = false, nil
	if store then ok, data = pcall(store.GetAsync, store, "p_" .. p.UserId) end
	local d = (ok and type(data) == "table") and data or default()
	d.total = d.total or 0; d.rep = d.rep or 0; d.served = d.served or 0; d.pending = nil; d.staff = d.staff or {}; d.upg = d.upg or {}; d.level = d.level or 1; d.decor = d.decor or { owned = {}, tent = "TentRed", chairs = "ChairPlastic", sign = "SignClassic" }; d.lastClaim = d.lastClaim or 0; d.streak = d.streak or 0
	D.cache[p.UserId] = d
	return d
end
function D.save(p)
